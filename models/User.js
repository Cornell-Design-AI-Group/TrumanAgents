...
const userSchema = new mongoose.Schema(
  {
    ...
    /**
     * Per-user game state
     * (kept here instead of global singletons)
     */
    levelState: {
      // Current level for this user (defaults to 1)
      level: { type: Number, default: 1 },
      // When the current level started (for per-user timer math)
      levelStartedAt: { type: Date, default: Date.now },
    },

    // Per-user health/score, tracked per level
    scores: [
      new Schema({
    ...
    // Per-user objective completion state
    objectiveProgress: [
      new Schema({
